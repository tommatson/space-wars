...
VIEW_BUILD_DIR = $(BUILD_DIR)/engine/imgui/view
...
VIEW_SRCS = $(wildcard $(VIEW_DIR)/*.cpp)
...
VIEW_OBJS = $(VIEW_SRCS:$(VIEW_DIR)/%.cpp=$(VIEW_BUILD_DIR)/%.o)
OBJS = $(GAME_SRC_OBJS) $(GAME_SCENES_OBJS) $(RENDERER_OBJS) $(SYSTEMS_OBJS) $(NETWORK_OBJS) $(SOCKETS_OBJS) $(SCENE_OBJS) $(CORE_OBJS) $(IMGUI_OBJS) $(VIEW_OBJS)
...
$(VIEW_BUILD_DIR)/%.o: $(VIEW_DIR)/%.cpp | dirs
	$(CXX) $(CXXFLAGS) -c $< -o $@

dirs:
	mkdir -p $(BUILD_DIR) $(RENDERER_BUILD_DIR) $(SYSTEMS_BUILD_DIR) $(NETWORK_BUILD_DIR) $(SOCKETS_BUILD_DIR) $(SCENE_BUILD_DIR) $(CORE_BUILD_DIR) $(IMGUI_BUILD_DIR) $(VIEW_BUILD_DIR) $(GAME_SRC_BUILD_DIR) $(GAME_SCENES_BUILD_DIR)
